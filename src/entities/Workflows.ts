...
export default class Workflows {
  ...
  private getTasksTree(firstTask: Task, tasks: Task[]): TaskNode {
    ...
    if (firstTask.taskConfig.nextBalanceConnector !== NULL_ADDRESS) {
      const nextTasks = tasks.filter(
        (t) => t.taskConfig.previousBalanceConnector === firstTask.taskConfig.nextBalanceConnector
      )
      ...
      childrenTasks.forEach((task) => {
        ...
        if (
          ...
          const grandChildrenTasks = tasks.filter(
            (t) => t.taskConfig.previousBalanceConnector === task.taskConfig.nextBalanceConnector
          )
  ...
  public get(treeIdx: number): {
    ...
    const traverse = (taskTree: TaskNode, xPosition: number, depth: number, parentId?: string) => {
      ...
      if (!visited.has(nodeId)) {
        ...
        taskTree.getChildren().forEach((child, index) => traverse(child, xPosition + index * 200, depth + 1, nodeId))
